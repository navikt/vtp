...
public class PersonBuilder {
    ...
    public static Person lagSøker() {
        return new Person(lagSøkerPersonopplysninger(), lagArbeidsforhold(), lagInntekt(), lagSøkerYtelser(), lagSkatteopplysninger());
    }
    ...
    public static Person lagAnnenPart() {
        return new Person(lagAnnenPartPersonopplysninger(),
    ...
    private static Personopplysninger lagSøkerPersonopplysninger() {
        var bostedsadresse = new Adresse(Adresse.AdresseType.BOSTEDSADRESSE, "0000001", CountryCode.NO, LocalDate.now().minusYears(1),
                null);
        var postadresse = new Adresse(Adresse.AdresseType.POSTADRESSE, "0000001", CountryCode.NL, LocalDate.now().minusYears(4),
                LocalDate.now().minusYears(1));
        var adresser = new Adresser(List.of(bostedsadresse, postadresse), Adressebeskyttelse.UGRADERT);

        return new Personopplysninger(new PersonIdent(SØKER_IDENT), UUID.randomUUID(), Rolle.MOR, new Navn("Dolly", null, "Duck"),
                ...
                List.of(new Familierelasjon(Familierelasjon.Relasjon.BARN, new PersonIdent(BARN1_IDENT)),
    ...
    private static Personopplysninger lagAnnenPartPersonopplysninger() {
        ...
        return new Personopplysninger(new PersonIdent(ANNEN_PART_IDENT), UUID.randomUUID(), Rolle.FAR,
                new Navn("Donald", null, "Duck"), LocalDate.now().minusYears(50), null, Språk.NB, Kjønn.M,
                new GeografiskTilknytning(CountryCode.NO, GeografiskTilknytning.GeografiskTilknytningType.LAND),
                List.of(new Familierelasjon(Familierelasjon.Relasjon.BARN, new PersonIdent(BARN1_IDENT)),
                        new Familierelasjon(Familierelasjon.Relasjon.BARN, new PersonIdent(BARN2_IDENT)),
                        new Familierelasjon(Familierelasjon.Relasjon.EKTE, new PersonIdent(SØKER_IDENT))),
                List.of(new Statsborgerskap(CountryCode.NO)), List.of(new Sivilstand(Sivilstand.Type.GIFT, null, null)),
                List.of(new Personstatus(Personstatus.Type.BOSA, LocalDate.now().minusYears(50), null)), List.of(), adresser, false);
    }
    ...
    public static Person lagBarn1() {
        return new Person(lagBarnPersonopplysninger(BARN1_IDENT, LocalDate.now().minusYears(3)), List.of(), List.of(), List.of(),
                List.of());
    }
    ...
    public static Person lagBarn2() {
        return new Person(lagBarnPersonopplysninger(BARN2_IDENT, LocalDate.now().minusYears(1)), List.of(), List.of(), List.of(),
                List.of());
    }
    ...
    private static Personopplysninger lagBarnPersonopplysninger(String ident, LocalDate fødselsdato) {
        var bostedsadresse = new Adresse(Adresse.AdresseType.BOSTEDSADRESSE, "0000001", CountryCode.NO, fødselsdato, null);
        var adresser = new Adresser(List.of(bostedsadresse), Adressebeskyttelse.UGRADERT);

        return new Personopplysninger(new PersonIdent(ident), UUID.randomUUID(), Rolle.BARN, new Navn("Lille", null, "Duck"),
                fødselsdato, null, Språk.NB, Kjønn.K,
                new GeografiskTilknytning(CountryCode.NO, GeografiskTilknytning.GeografiskTilknytningType.LAND),
                List.of(new Familierelasjon(Familierelasjon.Relasjon.MOR, new PersonIdent(SØKER_IDENT)),
                ...
                List.of(new Personstatus(Personstatus.Type.BOSA, fødselsdato, null)), List.of(), adresser, false);
    }
    ...
    private static List<Inntektsperiode> lagInntekt() {
        return List.of(
                new Inntektsperiode(new Organisasjon(new Orgnummer(ORGNR_FRILANS), "ARB001-003", null), LocalDate.now().minusYears(2),
                        LocalDate.now(), 45_000, Inntektsperiode.YtelseType.FASTLØNN, Inntektsperiode.FordelType.KONTANTYTELSE));
    }
    ...
    private static List<Ytelse> lagSøkerYtelser() {
        return List.of(
                new Ytelse(YtelseType.ARBEIDSAVKLARINGSPENGER, LocalDate.now().minusMonths(12), LocalDate.now().plusMonths(2), 1000,
                        10_000, List.of(new Beregningsgrunnlag(Beregningsgrunnlag.Kategori.ARBEIDSTAKER, null))));
    }
    ...
    private static List<Skatteopplysning> lagSkatteopplysninger() {
        return List.of(new Skatteopplysning(LocalDate.now().getYear() - 1, 550_000),

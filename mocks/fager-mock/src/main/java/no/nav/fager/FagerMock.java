...
public class FagerMock {
    ...
    public Map<String, Object> graphQLRequest(@HeaderParam(AUTHORIZATION) String authorizationHeader,
        ...
        if (LOG.isDebugEnabled()) {
            LOG.debug("FAGER: Operation name: {}", sanitizeInput(operationName));
            LOG.debug("FAGER: Query: {}", sanitizeInput(request.getQuery()));
        }
        ...
        if ("nySak".equals(operationName)) {
            ...
            var executionResult = graphqlTjeneste.sak(request);
            return executionResult.toSpecification();
        } else if ("nyBeskjed".equals(operationName)) {
            LOG.info("FAGER: nyBeskjed operation");
            var executionResult = graphqlTjeneste.beskjed(request);
            return executionResult.toSpecification();
        }

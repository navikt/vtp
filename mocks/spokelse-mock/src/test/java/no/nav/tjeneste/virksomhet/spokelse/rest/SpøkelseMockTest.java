...
class SpøkelseMockTest {
    ...
    void henterSykepengevedtakMedRiktigGradNaarSatt() {
        ...
        var sykepenger = new Ytelse(YtelseType.SYKEPENGER, LocalDate.now().minusMonths(2), LocalDate.now().plusMonths(1), 1500, null, 50, null, List.of());
    ...
    void bruker100ProsentGradSomDefaultNaarIkkeSatt() {
        var personBase = PersonBuilder.lagSøker();
        var sykepenger = new Ytelse(YtelseType.SYKEPENGER, LocalDate.now().minusMonths(2), LocalDate.now().plusMonths(1), 1500, null, null, null, List.of());
        var person = new Person(personBase.personopplysninger(), personBase.arbeidsforhold(), personBase.inntekt(), List.of(sykepenger), List.of());
        PersonRepository.leggTilPerson(person);
    ...
    @Test
    void filtrererBortYtelserMedEksplisittInfotrygdKilde() {
        var personBase = PersonBuilder.lagSøker();
        var sykepenger = new Ytelse(YtelseType.SYKEPENGER, LocalDate.now().minusMonths(2), LocalDate.now().plusMonths(1), 1500, null, 100, LegacyKilde.INFOTRYGD, List.of());
        var person = new Person(personBase.personopplysninger(), personBase.arbeidsforhold(), personBase.inntekt(), List.of(sykepenger), List.of());
        PersonRepository.leggTilPerson(person);
    ...
    void filtrererBortYtelserSomErAvsluttetFoerFomIRequest() {
        var personBase = PersonBuilder.lagSøker();
        var gammelSykepengeperiode = new Ytelse(YtelseType.SYKEPENGER, LocalDate.now().minusYears(2), LocalDate.now().minusYears(1), 1500, null, 100, null, List.of());

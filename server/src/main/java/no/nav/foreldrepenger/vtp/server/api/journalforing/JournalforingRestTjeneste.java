...
public class JournalforingRestTjeneste {
    ...
    public JournalforingResultatDto foreldrepengesoknadErketype(String xml,
                                                                @PathParam(AKTORID_KEY) String fnr,
                                                                @PathParam(DOKUMENTTYYPEID_KEY) DokumenttypeId dokumenttypeId){
    ...
    @GET
    @Produces(MediaType.APPLICATION_JSON)
    public DokumentModell hentDokument(@QueryParam(JOURNALPOST_ID) String dokumentId) {
        DokumentModell dokumentModell = journalRepository.finnDokumentMedDokumentId(dokumentId)
                .orElseThrow(() -> new NotFoundException("Finner ikke dokument med id " + dokumentId));
        LOG.info("Fant journalført dokumente med dokumentId: {}", dokumentId);
    ...
    public JournalforingResultatDto lagUstrukturertJournalpost(@PathParam(AKTORID_KEY) String fnr,
                                                               @PathParam(DOKUMENTTYYPEID_KEY) DokumenttypeId dokumenttypeid,
                                                               @QueryParam(JOURNALSTATUS) String journalstatus){

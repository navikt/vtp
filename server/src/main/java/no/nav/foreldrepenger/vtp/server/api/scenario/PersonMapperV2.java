...
public class PersonMapperV2 {
    ...
    private static Navn generertTilfeldigNavn(no.nav.foreldrepenger.vtp.kontrakter.person.Kjønn kjønn) {
        var generetNavn = no.nav.foreldrepenger.vtp.kontrakter.person.Kjønn.M.equals(kjønn)
    ...
    private static no.nav.vtp.person.personopplysninger.Språk tilSpråk(no.nav.foreldrepenger.vtp.kontrakter.person.Språk språk) {
        if (språk == null) {
            return null;
        }
        return switch (språk) {
    ...
    private static GeografiskTilknytning tilGeografiskTilknytning(GeografiskTilknytningDto dto) {
        ...
        return new GeografiskTilknytning(dto.land(), tilGeografiskTilknytningType(dto.type()));
    }
    ...
    private static List<Sivilstand> tilSivilstander(List<SivilstandDto> dtos) {
        ...
        return dtos.stream()
                .map(PersonMapperV2::tilSivilstand)
                .toList();
    }
    ...
    private static List<Medlemskap> tilMedlemskap(List<MedlemskapDto> dtos) {
        ...
        return dtos.stream()
                .map(dto -> new Medlemskap(dto.fom(), dto.tom(), dto.land(), tilDekningsType(dto.trygdedekning())))
                .toList();
    }
    ...
    private static Adresser tilAdresser(List<AdresseDto> adresser, no.nav.foreldrepenger.vtp.kontrakter.person.Adressebeskyttelse beskyttelse) {
        var mapped = adresser == null ? List.<Adresse>of() : adresser.stream().map(PersonMapperV2::tilAdresse).toList();
    ...
    private static no.nav.vtp.person.personopplysninger.Adressebeskyttelse tilAdressebeskyttelse(
            no.nav.foreldrepenger.vtp.kontrakter.person.Adressebeskyttelse beskyttelse) {
        if (beskyttelse == null) {
            return null;
        }
    ...
    private static List<Inntektsperiode> tilInntekt(List<InntektsperiodeDto> inntekt,
        ...
        var avledet = ytelser == null ? Stream.<Inntektsperiode>empty()
                : ytelser.stream()
                        .map(PersonMapperV2::tilInntektsperiodeFraYtelse)
